                        ...
                        <form action="{{ route('console.question.store') }}" method="POST">
                            ...
                            @foreach(['A','B','C','D','E'] as $i => $opt)
                                <div class="mb-2">
                                    ...
                                    {{-- Input text utk TWK & TKP --}}
                                    <input type="text" name="answers[{{ $i }}]" class="form-control answer-input" required >

                                    {{-- Textarea Summernote utk TIU --}}
                                    <textarea id="answer_{{ $opt }}" name="answers[{{ $i }}]" class="form-control answer-textarea d-none" rows="2"></textarea>
                                </div>
                            @endforeach
...
@push('js-bottom')
    <script>
        $(function () {
            ...
            // Inisialisasi summernote utk jawaban TIU
            ...
            // Validasi TKP sebelum submit
            $('form').on('submit', function(e){
                const cat = $("#topic_id option:selected").data("category");
                if (cat === 'TKP') {
                    let empty = false;
                    $('input[name="score_tkp[]"]').each(function(){
                        if ($(this).val() === '') empty = true;
                    });
                    if (empty) {
                        alert('Semua skor TKP wajib diisi!');
                        e.preventDefault();
                    }
                }
            });
        ...
        function updateScoreRules(){
            ...
            if (cat === 'TIU') {
                ...
                $('.answer-textarea').each(function(){
                    ...
                    .removeAttr('disabled')
                    .attr('required', true);
                    $(this).next('.note-editor').show(); // 🪄 tampilkan editor summernote
                });
            } else {
                ...
                $('.answer-textarea').each(function(){
                    ...
                    .attr('disabled', true)
                    .removeAttr('required');
                    $(this).next('.note-editor').hide(); // 🪄 sembunyikan editor summernote
                });
